package com.apitraining.Automation.util;
...
public class APIutil implements APIInterface{
	...
	String parameter = "id";
	String value = "1";
	String postpayload_str = "{'name' : 'Nischal','job' : 'Software Engineer'}";
	String patchpayload_str = "{'name' : 'Sri Nischal','job' : 'Senior Software Engineer'}";
	String putpayload_str = "{'name' : 'Suraj','job' : 'Senior Software Engineer'}";
	...
	public void apigetcall() {
		Response response = RestAssured.given().baseUri(url).param(parameter, value).when().get("/users").then().log()
	...
	public void apiputcall() {
		Response response = RestAssured.given().baseUri(url).body(putpayload_str.toString()).put("/users/1").then().log().all().extract().response();
		System.out.println(response);
	}
